...
def worker_qrcode(task_id):
    ...
    logger.info(f"QR SSE published task_id={task_id} has_image={has_image} "
                f"global_listeners={len(tq._global_listeners)}")
    return {"ok": True}
...
def worker_complete(task_id):
    ...
    # Basename strips path separators (prevent traversal); keep Unicode for Chinese filenames
    orig_name = os.path.basename(f.filename or "download").replace("\x00", "")
